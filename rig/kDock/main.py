...
def ui():
    ...
    dockLabel = 'K' # 너비를 최소화하기 위해
    dockID = 'dockingKSR_win'
    dockingKSR_win='dockingKSR_win'
    if mc.dockControl(dockID ,q=1,ex=1):
        mc.deleteUI(dockID)   
    ...
    dockControl = mc.dockControl(dockID, l=dockLabel, area='left', content=win, allowedArea=allowedAreas) # w=로는 조절이 안되는듯.
    
    # advanced skeleton 참조함. 강제로 너비를 줄어줌 맞춰줌. 이거 안하면 마야 실행시 너비가 넓어짐
    mc.dockControl(dockID, e=1, fixedWidth=1)
    mc.evalDeferred('mc.dockControl("%s", e=1, r=1)'%(dockID)) # look forward
